...
struct PlaceSearchServices {
    ...
    func getplaceSearchList(_ queryString: String, completion: @escaping ([PlaceSearchData]?) -> Void) {
        
        let url = HomeAPIs.placeSearchURL + String(describing: queryString)
        ...
        Alamofire.request(url, method: .get, parameters: .none, encoding: JSONEncoding.default, headers: header).responseJSON { response in
            ...
            case .success:
                ...
                do {
                    ...
                    let object = try decoder.decode(ResponseResult<PlaceSearchData>.self, from: data)
                    if object.success {
