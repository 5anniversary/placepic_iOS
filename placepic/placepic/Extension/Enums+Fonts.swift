//
//  Extensions+Fonts.swift
//  ToyProject@Crecker
//
//  Created by elesahich on 2020/06/23.
//  Copyright © 2020 elesahich. All rights reserved.
//

import UIKit

enum Font {
    static let fontSize11 = UIFont(name: "AppleSDGothicNeo-Regular", size: 11)
    static let fontSize12 = UIFont(name: "AppleSDGothicNeo-Regular", size: 12)
    static let fontSize13 = UIFont(name: "AppleSDGothicNeo-Regular", size: 13)
    static let fontSize14 = UIFont(name: "AppleSDGothicNeo-Regular", size: 14)
    static let fontSize15 = UIFont(name: "AppleSDGothicNeo-Regular", size: 15)
    static let fontSize16 = UIFont(name: "AppleSDGothicNeo-Regular", size: 16)
    static let fontSize17 = UIFont(name: "AppleSDGothicNeo-Regular", size: 17)
    static let fontSize18 = UIFont(name: "AppleSDGothicNeo-Regular", size: 18)
    static let fontSize19 = UIFont(name: "AppleSDGothicNeo-Regular", size: 19)
    static let fontSize20 = UIFont(name: "AppleSDGothicNeo-Regular", size: 20)

    static let boldFontSize11 = UIFont(name: "AppleSDGothicNeo-Bold", size: 11)
    static let boldFontSize12 = UIFont(name: "AppleSDGothicNeo-Bold", size: 12)
    static let boldFontSize13 = UIFont(name: "AppleSDGothicNeo-Bold", size: 13)
    static let boldFontSize14 = UIFont(name: "AppleSDGothicNeo-Bold", size: 14)
    static let boldFontSize15 = UIFont(name: "AppleSDGothicNeo-Bold", size: 15)
    static let boldFontSize16 = UIFont(name: "AppleSDGothicNeo-Bold", size: 16)
    static let boldFontSize17 = UIFont(name: "AppleSDGothicNeo-Bold", size: 17)
    static let boldFontSize18 = UIFont(name: "AppleSDGothicNeo-Bold", size: 18)
    static let boldFontSize20 = UIFont(name: "AppleSDGothicNeo-Bold", size: 20)
}
